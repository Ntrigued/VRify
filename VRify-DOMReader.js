...
VRify.getElemsToDisplay = function(head_id, elems_list = []) {
	...
	for(var i = 0; i < head_elem.children.length; i++) {
		Array.prototype.push.apply(elems_list, VRify.getElemsToDisplay(head_elem.children[i].id));
	}
	return elems_list;
}

VRify.getVRContent = function(base_elem_id){
        var content_list = VRify.getCardContent(base_elem_id);
        return content_list;
}

VRify.getCards = function(content_list_or_base_elem, size = 2) {
        if(typeof(content_list_or_base_elem) == typeof("")){
                content_list_or_base_elem = getVRContent(base_elem_id);
        }
        return VRify.partitionToCards(content_list_or_base_elem, size);
}

// Meant to be called with the output of VRify.getCardContent
VRify.partitionToCards = function(content_list, size=2) {
...
VRify.getCardContent = function(base_elem_id) {
        var elems_list = VRify.getElemsToDisplay(base_elem_id);
        ...
        elems_list.forEach(function(elem) {
                if(elem.tagName == "IMG")
                        ...
                        var text_list = VRify.parseText(elem);
...
VRify.parseText = function(text_elem) {
        var link_elems = []
